...
The component renders a PrometheusRule named `cilium-features` that emits one series for each managed feature via the `feature` label (`clustermesh`, `egress-gateway`, `transparent-encryption`) with value `1` when enabled and `0` when disabled, which makes it easy to use in alerts and/or dashboards.
