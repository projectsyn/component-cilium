...
[NOTE]
====
The component uses namespace `cilium` by default.

We don't recommend changing the parameter, but if you really need to adjust the namespace for some reason, it's possible with the following configuration for `install_method=helm`:

[source,yaml]
----
_want_nonstandard_namespace: true
_namespace: <desired namespace>
----

If you configure the namespace to `kube-system` through this mechanism, please note that the component won't generate a manifest for the namespace.
====
...
NOTE: This parameter has no effect when `olm.upgrade_strategy.generate_subscription=true`.

== `olm.migrate_to_clife`

[horizontal]
type:: bool
default:: `true`

Whether the component deploys an ArgoCD `PreSync` hook job which automates the migration from the old Cilium EE OLM operator (for Cilium <= 1.16) to the new CLife controller manager (for Cilium >= 1.17).

[TIP]
====
We recommend disabling this feature once all clusters in a Project Syn fleet are migrated to CLife.

Disabling this feature makes ArgoCD syncs faster and less complex.
====

=== `olm.generate_subscription`

[horizontal]
type:: bool
default:: `false`

When this parameter is set to true, the component deploys the OLM subscription provided by the upstream OLM tarball.

[WARNING]
====
It's not recommended to enable this feature before upgrading to Cilium 1.17 and migrating the new CLife OLM controller.

The automatic migration procedure from Cilium EE OLM to CLife that's provided by the component (see parameter `olm.migrate_to_clife`) doesn't work correctly when `generate_subscription=true`.
====

=== `olm.generate_olm_deployment`

[horizontal]
type:: bool
default:: `false`

This parameter has no effect unless `olm.generate_subscription=true`.
When this is set to true, the OLM manager deployment is rendered while `olm.generate_subscription=true`.
By default, this is set to `false`, since we don't want ArgoCD and OLM fighting over the OLM deployment after cluster bootstrapping.

[NOTE]
====
This must be set to `true` when this component is used to render the initial Cilium manifests which are copied into the openshift-installer's bootstrap manifests directory.
Otherwise, initial cluster bootstrapping will fail because Cilium won't get deployed during bootstrapping.
====

TIP: For VSHN Managed OpenShift, the install instructions and guided setup automatically set this parameter as necessary.


== `olm.upgrade_strategy`
...
This parameter controls how the component configures the Cilium OLM subscription, if `olm.generate_subscription=true`.
See the documentation for individual fields below.

=== `olm.upgrade_strategy.install_plan_approval`
...
This parameter allows users to customize whether OLM install plans are approved automatically or not.
By default, the component configures `Manual` approval to avoid Cilium restarts at arbitrary times when a new patch release is available.

NOTE: The component raises an error if a value other than `Automatic` or `Manual` is provided in this parameter.

=== `olm.upgrade_strategy.upgrade_job_hook`

[horizontal]
type:: bool
default:: `true`

When `olm.generate_subscription=true`, `olm.upgrade_strategy.install_plan_approval=Manual`, and this parameter is set to true, the component will attempt to deploy an https://github.com/appuio/openshift-upgrade-controller/[openshift-upgrade-controller] `UpgradeJobHook` which approves the latest pending install plan that was created before the active `UpgradeJob` was created.
This effectively implements Cilium patch upgrades as part of scheduled OpenShift maintenance windows.

The component will raise an error if this is set to true on a cluster where the OpenShift upgrade controller isn't deployed via Project Syn.

=== `olm.upgrade_strategy.upgrade_job_selector`

[horizontal]
type:: object
default:: `{}`

This parameter allows users to customize for which `UpgradeJob` resources the `UpgradeJobHook` is applied.
...
[NOTE]
====
The component will inject mandatory Helm values for certain Kubernetes distributions based on the value of inventory parameter `facts.distribution`.
Currently, the component knows distributions "OpenShift 4" (OCP or OKE) and "Talos".

For OpenShift 4, the component injects the following Helm values:

* `cni.binPath=/var/lib/cni/bin`
* `cni.confPath=/var/run/multus/cni/net.d`

For Talos, the component injects the following Helm values:

* `ipam.mode=kubernetes`
* `bpf.hostLegacyRouting=true` (if `bpf.masquerade=true`).
See this https://github.com/cilium/cilium/pull/36852[GitHub PR] for details regarding this configuration.
* `cgroup.autoMount.enabled=false` and `cgroup.hostRoot=/sys/fs/cgroup`
* reduced Linux capability sets (removing the `SYS_MODULE` capability for some Cilium agent containers).
====
...
It renders the alerts through the `alert-patching.libsonnet` and `prom.libsonnet` library aliases, which are provided by both https://github.com/appuio/component-openshift4-monitoring.git[component `openshift4-monitoring`] and https://github.com/projectsyn/component-prometheus.git[component `prometheus`].
If neither component is present on the target cluster, no alerts are rendered.
...
== `deploy_cert_manager_ca`

[horizontal]
type:: boolean
default:: `false`

This parameter controls whether the component deploys cert-manager `Issuer` resources and a CA certificate.
If enabled, the CA certificate is stored in secret `cilium-ca` in the Cilium namespace.
The CA certificate is configured with a lifetime of 2 years.

When the parameter is set to `true`, the component also deploys cert-manager resources to issue mTLS certificates for Hubble Relay and configures Hubble to use mTLS for connections between Hubble Relay and the Cilium agents.
