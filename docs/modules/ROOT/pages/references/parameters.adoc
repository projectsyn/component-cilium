...
The subscription is always patched to have `installPlanApproval=Manual`.
...
When `olm.upgrade_strategy.generate_subscription=true` and this parameter is set to true, the component will attempt to deploy an https://github.com/appuio/openshift-upgrade-controller/[openshift-upgrade-controller] `UpgradeJobHook` which automatically approves the latest pending install plan that was created before the active `UpgradeJob` was created.
