...
On other distributions, the component checks for the presence of https://github.com/projectsyn/component-prometheus.git[component `prometheus`], and uses the `prometheus.libsonnet` library provided by that component.
If the component is compiled for a cluster that's not OpenShift and which doesn't include `component-prometheus`, no alerts are rendered.

NOTE: Currently, `alerts.ignoreNames` and `alerts.patches` don't have any effect unless the component is compiled for OpenShift.
