...
generate_olm_deployment: false
upgrade_job_hook: true
upgrade_job_selector: {}
----

This parameter controls how the component deploys the Cilium OLM tarball.
See the documentation for individual fields below.

=== `olm.upgrade_strategy.generate_subscription`
...
=== `olm.upgrade_strategy.generate_olm_deployment`

[horizontal]
type:: bool
default:: `false`

This parameter has no effect unless `generate_subscription=true`.
When this is set to true, the OLM manager deployment is rendered while `generate_subscription=true`.
