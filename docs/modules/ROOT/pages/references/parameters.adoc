...
When the field is provide, and not an empty object, the component adds the contents as entries in `metadata.labels` of the resulting policies.
In this case, the component also configures the egress policies directly with the egress IP and `mmaxGatewayNodes: 1` in the `spec.egressGroups` entry.
